    ...
    def ExecuteFinalizeSolutionStep(self):
        ...
        if self.reference_case_name == "XFLR5":
            ...
            for section in sections:
                ...
                section_model_part = self.model.CreateModelPart(case_name)
                origin[1] = section/100.0 #* wing_span
                CPFApp.ComputeWingSectionVariableProcess(self.body_model_part, section_model_part, plane_normal, origin, -100.0, 100.0).Execute()
        ...
        if self.reference_case_name == "CRM":
            ...
            for section in sections:
                ...
                cp_ntf_file_name = 'references/cp/crm/ntf/ntf_' + str(section) + '.dat'
                if os.path.exists(cp_ntf_file_name):
                    x_potential = [float(line.split()[0]) for line in open(cp_ntf_file_name).readlines() if len(line.split()) > 0]
                    cp_potential = [float(line.split()[1]) for line in open(cp_ntf_file_name).readlines() if len(line.split()) > 0]
                    plt.plot(x_potential,cp_potential,'ks',label='WTD', markersize=5)
