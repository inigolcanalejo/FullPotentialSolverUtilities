...
for k in range(Number_Of_Domains_Size):
    ...
    for j in range(Number_Of_AOAS):
        ...
        for i in range(Number_Of_Refinements):
            ...
            # Create face for the wake
            Face_Wake = geompy.MakeFaceHW(Domain_Length*0.4, FarField_MeshSize*0.25, 1)

            # Move wake
            geompy.TranslateDXDYDZ(Face_Wake, Domain_Length*0.3, -math.sin(AOA*math.pi/180.0)*0.5, 0)

            # Fuse
            Fuse_1 = geompy.MakeFuseList([Cut_Domain, Face_Wake], True, False)

            # Explode
            [Edge_Inlet,Edge_Down,Edge_LowerSurface_LE,Edge_UpperSurface_LE,Edge_LowerSurface_TE,Edge_UpperSurface_TE,Edge_Up,Edge_Wake_LE,Edge_Wake_Down,Edge_Wake_Up,Edge_Outlet_Down,Edge_Outlet_Middle,Edge_Outlet_Up] = geompy.ExtractShapes(Fuse_1, geompy.ShapeType["EDGE"], True)

            #Body
            Body_Sub_mesh = geompy.CreateGroup(Fuse_1, geompy.ShapeType["EDGE"])
            ...
            geompy.addToStudyInFather( Fuse_1, Edge_Down, 'Edge_Down' )
            geompy.addToStudyInFather( Fuse_1, Edge_Inlet, 'Edge_Inlet' )
            geompy.addToStudy( Face_Domain, 'Face_Domain' )
            geompy.addToStudy( Face_Airfoil, 'Face_Airfoil' )
            geompy.addToStudyInFather( Fuse_1, Edge_LowerSurface_LE, 'Edge_LowerSurface_LE' )
            geompy.addToStudyInFather( Fuse_1, Edge_UpperSurface_LE, 'Edge_UpperSurface_LE' )
            geompy.addToStudy( Fuse_1, 'Fuse_1' )
            geompy.addToStudyInFather( Fuse_1, Edge_LowerSurface_TE, 'Edge_LowerSurface_TE' )
            geompy.addToStudyInFather( Fuse_1, Edge_UpperSurface_TE, 'Edge_UpperSurface_TE' )
            geompy.addToStudyInFather( Fuse_1, Edge_Wake_Down, 'Edge_Wake_Down' )
            geompy.addToStudyInFather( Fuse_1, Edge_Wake_Up, 'Edge_Wake_Up' )
            geompy.addToStudyInFather( Fuse_1, Auto_group_for_Sub_mesh_1_2, 'Auto_group_for_Sub-mesh_1' )
            geompy.addToStudyInFather( Fuse_1, Auto_group_for_Sub_mesh_Wake, 'Auto_group_for_Sub-mesh_Wake' )
            ...
            #NETGEN_1D_2D = smesh.CreateHypothesis('NETGEN_2D_ONLY', 'NETGENEngine')
            ...
            #Set wake mesh
            Regular_1D_3 = Fluid.Segment(geom=Auto_group_for_Sub_mesh_Wake)
            Local_Length_2 = Regular_1D_3.LocalLength(FarField_MeshSize*0.25,None,1e-07)
